    ...
    <section className="flex flex-col justify-center items-center bg-neutral-400 rounded">
      ...
      <div className="ml-2 md:w-1/2 lg:w-1/3">
        ...
        <div className="mt-8 flex justify-between mr-2 mb-2">
          <span className="bg-red-700 px-2 py-1 rounded text-neutral-200">{prato.category.label}</span>
          <span>{prato.size}g</span>
          <span>{prato.serving} pessoa{prato.serving > 1 ? "s" : ""}</span>
          <span className="font-bold text-red-700">R$ <span className="italic">{prato.price.toFixed(2).replace('.', ',')}</span></span>
        </div>
      </div>
    </section>
  )
